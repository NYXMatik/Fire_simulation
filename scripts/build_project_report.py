...
artifact. The files included in the artifact are listed in [Table 1](#table-1).
...
def reportlab_inline(text: str) -> str:
    ...
    for match in re.finditer(r"\[([^\]]+)\]\(([^)]+)\)", text):
        ...
        if url.startswith("#"):
            chunks.append(f'<link href="{url}" color="blue">{label}</link>')
        else:
    ...
    for kind, text in blocks:
        ...
        if kind == "h1":
            ...
            if text.startswith("Table "):
                anchor = '<a name="table-1"/> ' if text.startswith("Table 1.") else ""
                pending_table_caption = anchor + escaped
            else:
